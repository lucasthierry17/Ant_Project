...
class Ant(pg.sprite.Sprite):
    ...
    def update(self, dt):  # behavior
        ...
        wandrStr = 0.12  # how random they walk around
        ...
        if self.mode == 0 and self.pos.distance_to(self.nest) > 21:
            ...
            if (
                ...
                and scaledown_pos[0] in range(0, self.pgSize[0])
                and scaledown_pos[1] in range(0, self.pgSize[1])
            ):
            ...
            if left_GA_result == foodColor and right_GA_result != foodColor:
                ...
                self.desireDir = (
                    pg.Vector2(-1, 0).rotate(self.ang).normalize()
                )  # pg.Vector2(self.nest - self.pos).normalize()
            ...
            if self.pos.distance_to(self.nest) < 24:
                ...
                self.desireDir += pg.Vector2(self.nest - self.pos).normalize() * 0.08
                wandrStr = (
                    0.1  # pg.Vector2(self.desireDir + (1,0)).rotate(pg.math.Vector2.as_polar(self.nest - self.pos)[1])
                )
        ...
        accel = (
            dzStrFrc if pg.Vector2(dzStrFrc).magnitude() <= steerStr else pg.Vector2(dzStrFrc.normalize() * steerStr)
        )
