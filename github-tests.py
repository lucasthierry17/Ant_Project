    ...
    def update(self):
        ...
        if self.has_food:  # Ant has food
            ...
            if distance < (NEST_SIZE / 4):  # Ant has reached the nest
                ...
                # Adjust the desireDir to deviate from the direct path to the nest
                deviation_angle = random.uniform(-10, 10)
                self.desireDir = self.desireDir.rotate(deviation_angle)
                pheromone_value = 100
