...
class Ants(pygame.sprite.Sprite):
    ...
    def answer(self, scaled_pos):
        mid_result, left_result, right_result = [], [], []
        mid_sens = Vec2.vint(scaled_pos + pygame.Vector2(1, 0).rotate(self.angle))
        left_sens = Vec2.vint(scaled_pos + pygame.Vector2(2, -1).rotate(self.angle)) # -9
        right_sens = Vec2.vint(scaled_pos + pygame.Vector2(2, 1).rotate(self.angle))
        #print(scaled_pos)
        #print(mid_sens, right_sens, left_sens)
        ...
        if self.drawSurf.get_rect().collidepoint(left_sens):
            left_result = self.sensCheck(left_sens)
        if self.drawSurf.get_rect().collidepoint(right_sens):
            right_result = self.sensCheck(right_sens)
