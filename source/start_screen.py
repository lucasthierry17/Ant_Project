...
class StartMenu:
    """
    Initial menu for the Ant Search Simulation.
    - Manages the start menu display and user input for number of ants and speed.

    Methods:
    - __init__(): Initializes StartMenu with default settings.
    - initialize_start_menu(): Initialize fonts, texts, and other elements for the start menu.
    - draw(): Draws the start menu on the screen.
    - handle_events(): Handles key presses and mouse clicks for input.
    """
    def __init__(self) -> None:
        ...
        # status whether the input box was clicked
    ...
    def initialize_start_menu(self):
        ...
        # invisible rectangle to hover over and click start button
        self.start_button_rect = self.start_button.get_rect(center=(self.screen_width/2,
                                                                    self.screen_height - 80))

        # smaller font size
        self.settings_font = pygame.font.SysFont('Impact', 25)
        # heading of the ants input box
        self.ants_text = self.settings_font.render('Number of Ants', True, (255, 255, 255))
        # heading of the speed input
        self.speed_text = self.settings_font.render('Speed', True, (255, 255, 255))

        # font for the text in the input box
        self.textbox_font = pygame.font.SysFont('Arial',
    ...
    def draw(self):
        ...
        # draw title, start button and headings of the input boxes
        self.screen.blit(self.title,
                         (self.screen_width/2 - self.title.get_width()/2,
                          self.screen_height - (self.screen_height - 50)))
        self.screen.blit(self.start_button,
                         (self.screen_width/2 - self.start_button.get_width()/2,
                          self.screen_height - 100))
        self.screen.blit(self.ants_text,
                         (self.screen_width/2 - self.ants_text.get_width()/2,
                          self.screen_height - 320))
        self.screen.blit(self.speed_text,
                         (self.screen_width/2 - self.speed_text.get_width()/2,
                          self.screen_height - 230))

        white_text = (255,255,255)

        # input box for the user to type in the number of ants
        self.ants_box = pygame.Rect(self.screen_width / 2 - self.ants_text.get_width() / 2,
                                    self.screen_height - 270,
                                    self.ants_text.get_width(),
                                    self.ants_text.get_height())

        # input box for the speed of the ants
        self.speed_box = pygame.Rect(self.screen_width / 2 - self.ants_text.get_width() / 2,
        ...
        if self.ants_box_active:
            ...
            ants_cursor_x = self.ants_box.x + self.ants_text.get_width() / 2 + text_surface_ants.get_width() / 2
            self.ants_cursor.topleft = (ants_cursor_x, self.ants_box.y + 5)
        ...
        if self.speed_box_active:
            # cursor gets placed at the right side of the input
            speed_cursor_x = self.ants_box.x + self.ants_text.get_width() / 2 + text_surface_speed.get_width() / 2
            self.speed_cursor.topleft = (speed_cursor_x, self.speed_box.y + 5)
        ...
        self.screen.blit(self.value_range_ants,
                         (self.screen_width / 2 - self.value_range_ants.get_width() / 2,
                          self.screen_height - 290))
        self.screen.blit(self.value_range_speed,
                         (self.screen_width / 2 - self.value_range_speed.get_width() / 2,
